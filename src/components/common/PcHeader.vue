  ...
  <div :class="[headerClass, {bg_opacity:scroll}]" :disabled="$route.name==='notFound'" :title="($route.name==='auth-login'||$route.name==='bdc')&&'border'">
    <section class="box">
      <div class="nav_left1">
        ...
        <nav>
          <span v-if="$route.name === 'regist'">用户注册</span>
          <span v-else-if="$route.name === 'passwordRetrieval'">找回密码</span>
          <div :class="['item',{active:$route.name === i.name}]" v-for="(i,k) in nav" v-else>
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        nav: [
          {name: 'minerList', text: '算力服务器', link: '/miner'},
          {name: 'cloudComputeList', text: '云算力', link: '/cloudCompute'},
          {name: 'bdc', text: 'BDC托管', link: '/bdc'},
          {name: 'industryInfo', text: '产业资讯', link: '/industryInfo'}
        ],
        path: {
          fixed_header: ['login', 'bdc'],
          border: ['login', 'bdc'],
          shadow: ['regist', 'passwordRetrieval'],
          web_box: ['issues', 'issuesDetail', 'webInfoList', 'webInfoDetail', 'aboutUs', 'minerList', 'cloudComputeList']
        },
        headerClass: '',
        scroll: false
      }
      ...
      updateClass () {
        ...
        for (var i in this.path) {
          for (var j = 0; j < this.path[i].length; j++) {
            if (name === this.path[i][j]) {
              this.headerClass += i + ' '
              break
            }
          }
        }
        if (this.headerClass.indexOf('fixed_header') === -1) {
