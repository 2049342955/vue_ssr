...
class News extends Model
{
    ...
    {
        return $this->hasOne('App\Models\NewsClass', 'id', 'news_class_id');
    }
...
/************************************************************************/
    ...
    public function saveData($args)
    {
        ...
        $object->news_class_id = $args['news_class_id'];
    ...
    {
        $result = self::orderBy('dateline', 'desc')
                    ->orderBy('news_class_id', 'desc')
                    ->with('newsClass')
                    ->paginate($this->page);
        if (!empty($result)) {
            // 处理返回结果数据格式
